...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        Ok(ticket) => ticket,
        Err(ticketError) => match ticketError {
            TicketNewError::TitleError{error} => panic!("{error}"),
            TicketNewError::DescriptionError{description} => Ticket::new(title, description, status).unwrap(), 
        } 
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { error: "Title cannot be empty".to_string() } );
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { error: "Title cannot be longer than 50 bytes".to_string() } );
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { description: "Description not provided".to_string() });
        }
